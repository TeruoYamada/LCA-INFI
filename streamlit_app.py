...
# CORRIGIDO: adicionado parâmetro max_date para limitar o horizonte de previsão
# à data final definida pelo usuário, impedindo que a tendência extrapole além dela.
def predict_future_values(df, days=5, max_date=None):
    ...
    # Gera candidatos a até `days` dias à frente em passos de 6 horas
    ...
    # CORRECAO PRINCIPAL: descarta qualquer ponto além de max_date (end_date do usuário)
    if max_date is not None:
        max_dt = pd.Timestamp(max_date)
        # Garante o limite até o final do dia da data escolhida
...
# CORRIGIDO: recebe end_date e o repassa para predict_future_values
def analyze_all_cities(ds, pm25_var, pm10_var, cities_dict, end_date=None):
    ...
    for i, (city_name, coords) in enumerate(cities_dict.items()):
        ...
        if not df_timeseries.empty:
            # Passa end_date como max_date para restringir a janela de previsão
...
def generate_pm_analysis():
    ...
    try:
        ...
        with st.spinner("Gerando previsões..."):
            # CORRIGIDO: passa end_date como max_date para limitar a janela de previsão
            df_forecast = predict_future_values(df_timeseries, days=5, max_date=end_date)
        ...
        try:
            ...
            with st.spinner("Analisando qualidade do ar em todos os municípios de MS..."):
                # CORRIGIDO: passa end_date para que analyze_all_cities restrinja previsões
