...
def predict_future_aod(df, method='ensemble', days=5):
    ...
    if method == 'linear_regression':
        # Modelo de regressão linear simples (método original)
        ...
        df_pred = df_pred[['time', 'aod', 'type']]  # Manter compatibilidade
    ...
    with st.spinner(f"Analisando AOD para todos os municípios de MS... (0/{len(cities_dict)})"):
        for i, (city_name, coords) in enumerate(cities_dict.items()):
            ...
            if not df_timeseries.empty:
                # Gerar previsão usando método selecionado
...
def generate_aod_analysis():
    ...
    try:
        ...
        with st.spinner(f"Gerando previsão usando {method_options[forecast_method]['name']}..."):
            df_forecast = predict_future_aod(df_timeseries, method=forecast_method, days=5)
...
# Mostrar método selecionado na sidebar
if forecast_method in method_options:
    st.sidebar.success(f"Método Selecionado: {method_options[forecast_method]['name']}")
...
if st.button("🎞️ Gerar Análise Completa", type="primary", use_container_width=True):
    try:
        ...
        if results:
            ...
            # === ABA 2: COMPARAÇÃO DE MÉTODOS (só aparece se compare_methods=True) ===
            ...
            # === ABA 2/3: ALERTA DE POLUIÇÃO PARA MS ===
...
- **Animação Temporal**: Visualização da evolução espacial do AOD
- **Análise Comparativa**: Compare múltiplos métodos simultaneamente

### 🎯 Novos métodos de previsão:
- **🎯 Ensemble**: Combina todos os métodos para máxima precisão
- **🌳 Random Forest**: Machine learning com features temporais
- **🔄 Sazonal**: Detecta padrões diários e sazonais
- **📐 Polinomial**: Captura tendências não-lineares
- **🌊 Exponencial**: Pondera valores recentes
- **📊 Média Móvel**: Suaviza flutuações
- **📈 Linear**: Método clássico rápido

Desenvolvido para monitoramento avançado de aerossóis no estado de Mato Grosso do Sul - Brasil.
""")
